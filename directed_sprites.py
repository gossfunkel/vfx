...
if __name__ == "__main__":
    ...
    # 4 floats per sprite:
    # posx | posy | posz | xxxx
    raw_ssbo_data = np.zeros(sprite_bytes*NUM_SPRITES, dtype=np.float32)
    for sprite in range(NUM_SPRITES):
        raw_ssbo_data[sprite*sprite_bytes] = 50.*np.cos(360.*sprite/(NUM_SPRITES*2))
        raw_ssbo_data[sprite*sprite_bytes+2] = 25.*np.sin(360.*sprite/NUM_SPRITES)
    ...
    sprite_shader = Shader.load(Shader.SL_GLSL, "directed_sprites.vert", "directed_sprites.frag")
    ...
    compute_node.add_dispatch(NUM_SPRITES//8, 1, 1)
    compute_np = base.render.attach_new_node(compute_node)
    compute_np.set_shader(Shader.load_compute(Shader.SL_GLSL, "directed_sprites.comp"))
